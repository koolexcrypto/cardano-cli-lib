...
generateAddressKeys(cardanoCliParam:ICardanoCli.CardanoCliParam,addressKeyGenParam:ICardanoCli.AddressKeyGenParam):ICardanoCli.GeneratedAddressKeys {
...
buildAddress(cardanoCliParam:ICardanoCli.CardanoCliParam,addressKeyGenParam:ICardanoCli.AddressKeyGenParam,generatedAddressKeys:ICardanoCli.GeneratedAddressKeys):string {
...
buildRawTransaction(cardanoCliParam:ICardanoCli.CardanoCliParam,transaction:ICardanoCli.Transaction):void{
  const txIns = transaction.txIns.map( ( txIn , index) => {
    return ` --tx-in ${txIn.txHash}#${txIn.txIx}`;
  }).join("");
  const txOuts = transaction.txOuts.map( ( txOut , index) => {
...
calculaeMinFee(cardanoCliParam:ICardanoCli.CardanoCliParam,feeCalculationParams:ICardanoCli.FeeCalculationParams):number{
...
signTransction(cardanoCliParam:ICardanoCli.CardanoCliParam,transactionSignParams:ICardanoCli.TransactionSignParams):void{
